...
| 2 | Which 3–4 visualisations communicate the findings fastest? | Four: `ihds_phase1_eda.png` (what the data is really like), `ihds_shap_summary.png` (what drives the outcome), `ihds_business_translation.png` (what the model is worth), and `ihds_personas.png` (who the segments are). Rationale in [Visual appendix](#visual-appendix). |
...
**1. `results/ihds_phase1_eda.png` — "What is this data really like?"**
...
**2. `results/ihds_shap_summary.png` — "What drives the outcome?"**
The SHAP summary. `Log_Income` dominates by 4.5×, which is the single most important thing for a stakeholder to internalise before believing any spending-behaviour story. Paired with `ihds_shap_dependence.png` when the counter-intuitive grocery finding needs explaining.

**3. `results/ihds_business_translation.png` — "Is the model worth deploying?"**
...
**4. `results/ihds_personas.png` — "Who are the segments?"**
...
**Deliberately excluded:** `ihds_model_comparison.png` and `ihds_cluster_selection.png`. Both are model-selection diagnostics that answer questions a technical reviewer asks and a stakeholder does not. They belong in Phases 4 and 6, not in a summary deck.
