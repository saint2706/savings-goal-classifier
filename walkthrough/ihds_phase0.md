...
**Why 20%.** It is a convention, not a measurement — roughly the savings rate implied by common personal-finance guidance and close to India's household savings rate in the survey period. **It was chosen before the results were seen, and it is configurable** (`--threshold` in `src/build_ihds_dataset.py`). Phase 1 published the full sensitivity curve precisely so no reader has to take the choice on trust:
