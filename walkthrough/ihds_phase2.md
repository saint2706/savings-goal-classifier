...
**Notebook:** [`notebooks/ihds_02_feature_engineering.ipynb`](../notebooks/ihds_02_feature_engineering.ipynb)
**Builds on:** [Phase 1 (IHDS-II)](ihds_phase1.md), [Migration](ihds_migration.md)
**Output:** `dataset/ihds2_features.csv` — 41,518 households × 28 features
...
**Outcome:** CLR is excluded from the classification feature set. The six CLR columns are still written to `ihds2_features.csv` for **Phase 6** to evaluate, because clustering is the step that genuinely needs a metric on the simplex — but Phase 6 should test ILR rather than CLR, for the singularity reason above.
...
| Phase | Consequence |
| --- | --- |
| **3 — Baseline** | Load `dataset/ihds2_features.csv` directly. Majority baseline: 0.681 accuracy / 0.405 macro-F1. |
